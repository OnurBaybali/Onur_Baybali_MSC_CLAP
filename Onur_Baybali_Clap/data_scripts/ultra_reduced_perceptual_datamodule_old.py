# retrieval/data_handling/ultra_reduced_perceptual_datamodule.py
...
class UltraReducedPerceptualDataset(Dataset):
    """
    PKL → direkt feature tensörleri (40 features)
    ...
    def __init__(self, pkl_path, tokenizer_name="bert-base-uncased", split="train"):
        ...
        self.audio_feats = self.data["features"]  # (N, 40) direkt tensor
    ...
    def __getitem__(self, idx):
        ...
        return (
            torch.tensor(audio_feat, dtype=torch.float32),  # (40,)
...
def collate_fn(batch):
    ...
    return (
        torch.stack(audio),        # (B, 40) float32
        torch.stack(input_ids),    # (B, L) long
        torch.stack(idx)           # (B,)   long
    )


class UltraReducedPerceptualDataModule:
    def __init__(self, cfg):
        ...
        self.train_dataset = UltraReducedPerceptualDataset(self.paths["train_pkl"], self.tokenizer_name, "train")
        self.val_dataset   = UltraReducedPerceptualDataset(self.paths["val_pkl"],   self.tokenizer_name, "validation")
        self.test_dataset  = UltraReducedPerceptualDataset(self.paths["test_pkl"],  self.tokenizer_name, "test")

        # feature_dim auto
        self.feature_dim = self.train_dataset[0][0].numel()
        print(f"[UltraReducedPerceptualDataModule] inferred feature_dim: {self.feature_dim}")
