# retrieval/data_handling/perceptual_datamodule.py
# OLD VERSION (Before fixed_caption_pair fix)
# BUG: Returns caption idx instead of audio_idx, causing incorrect positive pair matching
...
class PerceptualDataset(Dataset):
    """
    PKL → direct feature tensors
    Reads captions from Clotho JSON files.
    Tokenizer: bert-base-uncased (from config)
    Returns: (audio_feat_tensor, input_ids_tensor, idx_tensor)
    
    NOTE: This version has a bug - returns caption idx instead of audio_idx
    This causes incorrect positive pair matching during training.
    """
    def __init__(self, pkl_path, tokenizer_name="bert-base-uncased", split="train"):
        ...
        self.audio_feats = self.data["features"]  # (N, D) direkt tensor
        ...
        if len(self.captions) != exp_caps:
            print(f"[WARN] {split} split: caption sayısı {len(self.captions)}; "
                  ...
                  "JSON and PKL lists may not match.")
    ...
    def __getitem__(self, idx):
        ...
        tokens = self.tokenizer(
            ...
            max_length=30,
            return_tensors="pt",
            return_attention_mask=False  # No need for mask; encoder generates it internally
        )
        # squeeze batch dim
        input_ids = tokens["input_ids"].squeeze(0)

        return (
            torch.tensor(audio_feat, dtype=torch.float32),  # (F,)
            input_ids.to(dtype=torch.long),                 # (L,)
            torch.tensor(idx, dtype=torch.long)             # () - BUG: Returns caption idx, not audio_idx!
        )
...
def collate_fn(batch):
    ...
    return (
        torch.stack(audio),        # (B, F) float32
        torch.stack(input_ids),    # (B, L) long
        torch.stack(idx)           # (B,)   long
    )


class PerceptualDataModule:
    def __init__(self, cfg):
        ...
        self.train_dataset = PerceptualDataset(self.paths["train_pkl"], self.tokenizer_name, "train")
        self.val_dataset   = PerceptualDataset(self.paths["val_pkl"],   self.tokenizer_name, "validation")
        self.test_dataset  = PerceptualDataset(self.paths["test_pkl"],  self.tokenizer_name, "test")

        # feature_dim auto
        self.feature_dim = self.train_dataset[0][0].numel()
        print(f"[DataModule] inferred feature_dim: {self.feature_dim}")
